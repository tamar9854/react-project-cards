...
function EditCard() {
    ...
    const getCard = async () => {
        const data = await cardService.getCardbyId(params.cardId)

        const { _id, __v, address, image, likes, createdAt, ...card } = data.data
        setId(_id)
        card.address = removeId(address);
        card.image = removeId(image);
        setValues(card)
    }
    const removeId = (obj) => {
        const { _id, ...base } = obj;
    ...
    const { getFieldProps, touched, errors, handleSubmit, resetForm, setValues } = useFormik({
        initialValues: {
            title: params.cardId,
            subtitle: "",
            description: "",
            ...
            const schema = Joi.object({
                ...
                bizNumber: Joi.number().required().label("business number"),
            ...
            <form className="m-3 p-3 mt-1 pt-1" onSubmit={handleSubmit}>
                <div className="row">
                    <div className="jumbotron">
                        <h4 className="text-center">Edit Card</h4>
                    </div>
                </div>
                ...
                <div className="row mb-5">
                    <div className="col-sm-12 text-center">
                        <Button type="submit" disabled={Object.keys(errors).length > 0} value="Save Card" />
                    </div>
                </div>
            </form>
        </div>
